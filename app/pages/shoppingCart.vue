        ...
        <div class="content" v-if="getShoppingCart.length > 0">
                ...
                <li class="product-list" v-for="(item, index) in getShoppingCart" :key="item.id">
                    ...
                    <div class="pro-num">
                        <el-input-number size="small" v-model="item.num"
                            @change="handleChange($event, index, item.productID)" :min="1"
                            :max="item.maxNum"></el-input-number>
                    </div>
                    ...
                    <div class="pro-action">
                        <el-popover placement="right">
                            ...
                            <template #reference>
                                <div><el-icon><Delete /></el-icon></div>
            ...
            <div class="cart-bar">
                <div class="cart-bar-left">
                    <span>
                        <nuxt-link to="/goods">继续购物</nuxt-link>
                    </span>
                    <span class="sep">|</span>
                ...
                <div class="cart-bar-right">
                    ...
                    <nuxt-link :to="getCheckNum > 0 ? '/confirmOrder' : ''">
...
const shopCart = shoppingCartStore();
const { updateShoppingCart, deleteShoppingCart, checkAll } = shopCart;
const { getShoppingCart, getCheckNum, getTotalPrice, getNum, getIsAllCheck } =
    storeToRefs(shopCart);
const net = request();
const $target = computed(() => runtimeConfig.public.baseUrl+'/');

const isAllCheck = computed({
    get() {
        return getIsAllCheck.value;
    },
    set(val) {
        console.log('val', val);
        checkAll(val);
...
async function handleChange(currentValue, key, productID) {
    updateShoppingCart({ key: key, prop: "check", val: true });
    let res = await net.post("/api/user/shoppingCart/updateShoppingCart", {
        body: {
...
function checkChange(val, key) {
    updateShoppingCart({ key: key, prop: "check", val: val });
}
async function deleteItem(e, id, productID) {
    let res = await net.post("/api/user/shoppingCart/deleteShoppingCart", {
        body: {
            user_id: userInfo.getUser.user_id,
            product_id: productID,
        },
