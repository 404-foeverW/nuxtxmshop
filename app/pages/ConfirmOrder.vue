    ...
    <div class="confirmOrder">
        ...
        <div class="content">
            ...
            <div class="section-bar">
                <div class="btn">
                    <nuxt-link to="/shoppingCart" class="btn-base btn-return">返回购物车</nuxt-link>
...
const { getCheckNum, getTotalPrice, getCheckGoods } = storeToRefs(shopCart);
const { deleteShoppingCart } = shopCart;
const userInfo = userStore();
const { getUser } = storeToRefs(userInfo);
...
function selectAddr(id) {
    confirmAddress.value = id;
}
async function addOrder() {
    let res = await net.post('/api/user/order/addOrder', {
        body: {
            user_id: getUser.value.user_id,
            products: getCheckGoods.value
        }
    })
    let products = getCheckGoods.value;
    switch (res.code) {
        // “001”代表结算成功
        case "001":
